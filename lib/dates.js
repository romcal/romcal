// Load relevant resources and libs 
...
// Preload the date cache
var fs = require('fs'),
    cacheFile = Path.join( __dirname, '../data/cache/dates.json' ),
    cache = JSON.parse( fs.readFileSync( cacheFile, 'utf8' ) );

// Flag to determine if cache should be updated with date
// calculation results (should always be false unless updating
// the module code)
...
var _epiphany = function() {
  ...
  var cacheKey = 'epiphany|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {

    // Get the first day of the year
    var firstDay = moment.utc({ year: arguments[0], month: 0, day: 1 }),
        date = moment.utc({ year: arguments[0], month: 0, day: 6 });
    ...
    if ( traditional ) {
      ...
      if ( UPDATE_CACHE ) {
        _.set( cache, cacheKey, date.valueOf() );
...
var _christmas = function() {
  ...
  var cacheKey = 'christmas|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {

    var date = moment.utc({ year: arguments[0], month: 11, day: 25 });
...
var _maryMotherOfGod = function() {
  ...
  var cacheKey = 'maryMotherOfGod|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {
    
    var date = moment.utc({ year: arguments[0], month: 0, day: 1 });
...
var _baptismOfTheLord = function() {
  ...
  var cacheKey = 'baptismOfTheLord|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {
...
var _presentationOfTheLord = function() {
  ...
  var cacheKey = 'presentationOfTheLord|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {
    
    var date = moment.utc({ year: arguments[0], month: 1, day: 2 });
...
var _christTheKing = function() {
  ...
  var cacheKey = 'christTheKing|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {
    
    var date = _firstSundayOfAdvent.apply( this, arguments ).subtract( 7, 'days').startOf('day');
...
var _ashWednesday = function() {
  ...
  var cacheKey = 'ashWednesday|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {
    
    var date = _easter.apply( this, arguments ).subtract( 46, 'days' ).startOf('day');
...
var _palmSunday = function() {
  ...
  var cacheKey = 'palmSunday|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {

    var date = _easter.apply( this, arguments ).subtract( 7, 'days' ).startOf('day');
...
var _holyThursday = function() {
  ...
  var cacheKey = 'holyThursday|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {

    var date = _easter.apply( this, arguments ).subtract( 3, 'days' ).startOf('day');
...
var _goodFriday = function() {
  ...
  var cacheKey = 'goodFriday|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {

    var date = _easter.apply( this, arguments ).subtract( 2, 'days' ).startOf('day');
...
var _holySaturday = function() {
  ...
  var cacheKey = 'holySaturday|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {

    var date = _easter.apply( this, arguments ).subtract( 1, 'days' ).startOf('day');
...
var _easter = function() {
  ...
  var cacheKey = 'easter|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {
    ...
    var date = moment.utc({ year: Y, month: ( M - 1 ), day: D });
...
var _divineMercySunday = function() {
  ...
  var cacheKey = 'divineMercySunday|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {

    var date = _easter.apply( this, arguments ).add( 7, 'days' ).startOf('day');
...
var _pentecostSunday = function() {
  ...
  var cacheKey = 'pentecostSunday|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {
    var date = _easter.apply( this, arguments ).add( 49, 'days' ).startOf('day');
...
var _firstSundayOfAdvent = function() {
  ...
  var cacheKey = 'firstSundayOfAdvent|' + Array.prototype.slice.call(arguments).join('|');
  if ( _.has( cache, cacheKey ) ) {
    return moment.utc( _.get( cache, cacheKey ) );
  }
  else {
    ...
    switch( christmas.day() ) {
      case 0: // Sunday
        date = moment.utc({ year: arguments[0], month: 10, day: 27 });
        break;
      case 1: // Monday
        date = moment.utc({ year: arguments[0], month: 11, day: 3 });
        break;
      case 2: // Tuesday
        date = moment.utc({ year: arguments[0], month: 11, day: 2 });
        break;
      case 3: // Wednesday
        date = moment.utc({ year: arguments[0], month: 11, day: 1 });
        break;
      case 4: // Thursday
        date = moment.utc({ year: arguments[0], month: 10, day: 30 });
        break;
      case 5: // Friday
        date = moment.utc({ year: arguments[0], month: 10, day: 29 });
        break;
      case 6: // Saturday
        date = moment.utc({ year: arguments[0], month: 10, day: 28 });
        break;
      default:
        break;
    }
...
module.exports = {
  ...
  josephHusbandOfMary: function() {
    ...
    var cacheKey = 'josephHusbandOfMary|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
  ...
  annunciation: function() {
    ...
    var cacheKey = 'annunciation|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
  ...
  birthOfJohnTheBaptist: function() {
    ...
    var cacheKey = 'birthOfJohnTheBaptist|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {

      var date = moment.utc({ year: arguments[0], month: 5, day: 24 });
  ...
  peterAndPaulApostles: function() {
    ...
    var cacheKey = 'peterAndPaulApostles|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
      var date = moment.utc({ year: arguments[0], month: 5, day: 29 });
  ...
  assumption: function() {
    ...
    var cacheKey = 'assumption|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
      
      var date = moment.utc({ year: arguments[0], month: 7, day: 15 });
  ...
  allSaints: function() {
    ...
    var cacheKey = 'allSaints|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {

      var date = moment.utc({ year: arguments[0], month: 10, day: 1 });
  ...
  immaculateConception: function() {
    ...
    var cacheKey = 'immaculateConception|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
  ...
  ascension: function() {
    ...
    var cacheKey = 'ascension|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
      ...
      if ( !_.isUndefined( arguments[1] ) && arguments[1] ) {
        date = _easter( arguments[0] ).add( 42, 'days' );
      }
      // By default, Ascension on Thursday
      else {
        date = _easter( arguments[0] ).add( 39, 'days' );
      }
  ...
  trinitySunday: function() {
    ...
    var cacheKey = 'trinitySunday|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
      
      var date = _easter( arguments[0] ).add( 56, 'days' );
  ...
  corpusChristi: function() {
    var cacheKey = 'corpusChristi|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
      ...
      if ( !_.isUndefined( arguments[1] ) && arguments[1] ) {
        date = _easter( arguments[0] ).add( 60, 'days' );
      }
      // By default Corpus Christi on Sunday
      else {
        date = _easter( arguments[0] ).add( 63, 'days' );
      }
  ...
  sacredHeartOfJesus: function() {
    var cacheKey = 'sacredHeartOfJesus|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
      var date = _easter( arguments[0] ).add( 68, 'days' );
      if ( UPDATE_CACHE ) {
        _.set( cache, cacheKey, date.valueOf() );
        fs.writeFileSync( cacheFile, JSON.stringify( cache ), 'utf8' );
      }
  ...
  immaculateHeartOfMary: function() {
    var cacheKey = 'immaculateHeartOfMary|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
      var date = _easter( arguments[0] ).add( 69, 'days' );
      if ( UPDATE_CACHE ) {
        _.set( cache, cacheKey, date.valueOf() );
        fs.writeFileSync( cacheFile, JSON.stringify( cache ), 'utf8' );
      }
  ...
  // arguments[0]: year
  holyFamily: function() {
    var cacheKey = 'holyFamily|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
      ...
      if ( _.eq( christmas.day(), 0 ) ) {
        date = moment.utc({ year: arguments[0], month: 11, day: 30 });
      }
      // Holy Family is 1 week after Christmas when Christmas is on a Weekday
      else {
        date = christmas.add( 1, 'weeks' ).startOf('week');
      }
  ...
  transfiguration: function() {
    var cacheKey = 'transfiguration|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
      var date = moment.utc({ year: arguments[0], month: 7, day: 6 });
      if ( UPDATE_CACHE ) {
        _.set( cache, cacheKey, date.valueOf() );
        fs.writeFileSync( cacheFile, JSON.stringify( cache ), 'utf8' );
      }
  ...
  triumphOfTheCross: function() {
    var cacheKey = 'triumphOfTheCross|' + Array.prototype.slice.call(arguments).join('|');
    if ( _.has( cache, cacheKey ) ) {
      return moment.utc( _.get( cache, cacheKey ) );
    }
    else {
      var date = moment.utc({ year: arguments[0], month: 8, day: 14 });
      if ( UPDATE_CACHE ) {
        _.set( cache, cacheKey, date.valueOf() );
        fs.writeFileSync( cacheFile, JSON.stringify( cache ), 'utf8' );
      }
