...
describe('Switzerland national overlay (Nicholas of Flüe)', () => {
  ...
  test('Nicholas of Flüe propagates to every diocesan overlay via parent inheritance', async () => {
    for (const overlay of [
      ...
      const day = await r.resolveDate('1962-09-25');
      expect(day).toBeDefined();
      expect(day![0].key).toBe('saint_nicholas_of_flue_hermit_patron_of_switzerland');
      expect(day![0].name).toMatch(/Niklaus von Flüe/);
    }
  });
});
...
describe('Diocese of Basel', () => {
  test('Sts Ursus & Victor (Sep 30) are ClassI, outranking universal St Jerome', async () => {
    ...
    const day = await r.resolveDate('1962-09-30');
    expect(day![0].key).toBe('saints_ursus_and_victor_of_solothurn_martyrs_patrons');
    expect(day![0].rank1962).toBe('ClassI');
    expect(day!.slice(1).some((c) => c.key === 'saint_jerome_priest_confessor_and_doctor_of_the_church')).toBe(true);
  });

  test('St Ursicinus (Dec 20) is commemorated on the Advent feria (ClassII outranks ClassIII)', async () => {
    const r = new Romcal1962({ calendar: Switzerland_Basel });
    const day = await r.resolveDate('1962-12-20');
    ...
    expect(day![0].key).toBe('advent_3_thursday');
    expect(day!.slice(1).some((c) => c.key === 'saint_ursicinus_of_saint_ursanne_abbot')).toBe(true);
  });
});
...
describe('Diocese of Sankt Gallen', () => {
  test('St Gallus (Oct 16) is ClassI principal patron', async () => {
    ...
    const day = await r.resolveDate('1962-10-16');
    expect(day![0].key).toBe('saint_gallus_abbot_patron_of_the_diocese');
    expect(day![0].rank1962).toBe('ClassI');
  });

  test('St Othmar (Nov 16) is ClassII co-patron, outranking universal St Gertrude', async () => {
    const r = new Romcal1962({ calendar: Switzerland_Sankt_Gallen });
    const day = await r.resolveDate('1962-11-16');
    expect(day![0].key).toBe('saint_otmar_abbot_copatron_of_the_diocese');
    expect(day![0].rank1962).toBe('ClassII');
  });
});
...
describe('Diocese of Sion (Sitten)', () => {
  test('St Theodore/Théodule (Aug 16) is ClassI principal patron, outranking St Joachim', async () => {
    ...
    const day = await r.resolveDate('1962-08-16');
    expect(day![0].key).toBe('saint_theodore_of_octodurus_bishop_patron_of_the_diocese');
    expect(day![0].rank1962).toBe('ClassI');
    expect(day!.slice(1).some((c) => c.key === 'saint_joachim_confessor_father_of_the_blessed_virgin_mary')).toBe(true);
  });
});
...
describe('Diocese of Lugano', () => {
  ...
  test('Charles Borromeo Nov 4 primary in Lugano carries the overlay patron name', async () => {
    ...
    const day = await r.resolveDate('1962-11-04');
    expect(day![0].name).toMatch(/Carlo Borromeo/);
    expect(day![0].name).toMatch(/Patrono/);
  });

  test('St Abbondio (Aug 31) is ClassII secondary patron', async () => {
    const r = new Romcal1962({ calendar: Switzerland_Lugano });
    const day = await r.resolveDate('1962-08-31');
    expect(day![0].key).toBe('saint_abundius_of_como_bishop_copatron_of_the_diocese');
    expect(day![0].rank1962).toBe('ClassII');
  });
});
...
describe('Diocese of Lausanne–Geneva–Fribourg', () => {
  ...
  test('Dec 6 primary is St Nicholas with the patron suffix in French locale', async () => {
    ...
    const day = await r.resolveDate('1962-12-06');
    expect(day![0].name).toMatch(/Nicolas/);
    expect(day![0].name).toMatch(/Patron principal/);
  });
});
...
describe('Territorial Abbey of Saint-Maurice', () => {
  test('Ss Maurice & Companions (Sep 22) raised to ClassI for the abbey', async () => {
    ...
    const day = await r.resolveDate('1962-09-22');
    expect(day![0].key).toBe('ss_maurice_and_companions_martyrs_optional');
    expect(day![0].rank1962).toBe('ClassI');
    // St Thomas of Villanova (universal ClassIII for this date) drops to commemoration.
    expect(day!.slice(1).some((c) => c.key === 'saint_thomas_of_villanova_bishop_and_confessor')).toBe(true);
  });
});
...
describe('Registry + backwards compatibility', () => {
  ...
  test('Romcal1962 with no overlay still returns the pure universal calendar on Sep 25', async () => {
    ...
    if (day) {
      expect(day[0].key).not.toBe('saint_nicholas_of_flue_hermit_patron_of_switzerland');
    }
  });
});
