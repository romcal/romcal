...
modules under `src/locales/{lang}.ts`; Mass-proper text lives in
`data/propers/{lang}/{source}.json` and is spliced into a shared structural
scaffold at lookup time.

Fallback chain: `localeId → en → la`. Latin is always available. Supported locales:
...
- `key`, `name`, `kind: 'tempora' | 'sancti'`, `classOf1962` (1–4),
  `precedence1962` (15-slot `Precedences1962`, Rubricae 1960 §91), `colors`,
  `rubrics`.
...
- **Rubrical framework:** *Rubricae Generales Breviarii et Missalis Romani*
  (Codex Rubricarum, 1960) — occurrence, commemoration, transfer, and
  precedence rules (§91 tabula dierum liturgicorum, §15 Sunday-
  displacement, §50 forward transfer, §111–113 commemoration filter).
- **Kalendar + Mass propers seed corpus:** [Divinum Officium](https://divinumofficium.com)
  (`rubricis 1960` branch), imported once to populate
  `data/calendar-1960.json`, `data/sancti.json`, `data/tempora.json`,
  `data/commune.json`, and `data/propers/{lang}/*.json`. The JSON files
  are now hand-edited; the original importer has been removed.
