...
var Utils = require('./utils'),
    ...
    Season = require('../data/seasons.json'),
    Titles = require('../data/titles.json'),
    PsalterWeeks = require('../data/psalterWeeks.json'),
...
var _getConfig = function() {
  ...
  o.country = o.country || 'unitedStates';
    ...
    .map( function( coincidences ) {
      ...
      if ( coincidences.length > 1 ) {
        ...
        // If the group has a celebration and national date and no general date
          ...
          if ( _.first( sources['n'] ).data.prioritized ) {
            keep = 'n';
          }
        ...
        // If the group has a national and general date but no celebration
          ...
          if ( !_.first( sources['n'] ).data.prioritized && _.first( sources['g'] ).data.prioritized ) {
            keep = 'g';
          }
          else {
            keep = 'n';
  ...
  _.map( liturgicalDates, function( date, timestamp ) {
    ...
    if ( _.has( calendarDates, timestamp ) ) {
      ...
      //------------------------------------------------------------------
        ...
        //------------------------------------------------------------------
        if ( 
          _.eq( candidate.type, _.first( Types ) ) 
        ...
        // else {
        //   console.log('unhandled:', candidate.type, candidate.moment.format('L'), candidate.name );
        // }
...
var _calendarFor = function() {
  ...
  if ( !_.isNull( query ) && !_.isEmpty( query ) ) {
    ...
    if ( _.has( query, 'title' ) ) {
      dates = _.filter( dates, function( d ) {
        return _.contains( d.data.meta.titles, Titles[ _.get( query, 'title' ) ] );
      });
    }
  }
