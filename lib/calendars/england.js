  ...
  dates: function() {
    ...
    return [
      ...
      // generally the Monday of the Second Week of Easter.
      {
        ...
        "type": Types[0],
        "moment": (function( y ) {
          var easterTriduum = _.pluck( Seasons.easterTriduum( y ), 'moment' ),
              easterOctave = _.pluck( Seasons.easterOctave( y ), 'moment' ),
              easterTriduumRange = moment.range( _.first( easterTriduum ), _.last( easterTriduum ) ),
              easterOctaveRange = moment.range( _.first( easterOctave ), _.last( easterOctave ) ),
              date = moment.utc({ year: y, month: 3, day: 23 });

          // If the celebration lands anywhere between Easter Triduum to Divine Mercy Sunday (inclusive)
          // move it to the Monday after Divine Mercy Sunday
          if ( easterTriduumRange.contains( date ) || easterOctaveRange.contains( date ) ) {
            return _.last( easterOctave ).add( 1, 'days' );
          }
          else {
            return date;
          }
      ...
      },
      {
        ...
        "name": "Saint Adalbert, Bishop and Martyr/Saint Fidelis of Sigmaringen, Priest and Martyr",
        "type": Types[6],
