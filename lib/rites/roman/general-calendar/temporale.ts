...
export default class Temporale {
   ...
   * @private
   */
  #addLiturgicalCycleMetadata(date: Dayjs, calendar: RomcalCalendarMetadata): RomcalCyclesMetadata {
  ...
  /**
   * Compute the calendar metadata of each date definitions
   * @private
   * @param year
   * @param date
   * @param startOfSeason
  ...
  #computeCalendarMetadata = (
    ...
    if (!this.#calendarCache[year]) {
      ...
      const endOfLiturgicalYear = Dates.firstSundayOfAdvent(year + 1).subtract(1, 'day');
   ...
   */
  #getWeekOfChristmasTime = (date: Dayjs): number => {
  ...
  adventBuilder = (year: number): LiturgicalDefBuiltData => {
    ...
    const byKeys = datesOfAdvent.reduce((acc: Record<string, LiturgicalDay>, date: Dayjs, idx) => {
      const week = Math.floor(idx / 7) + 1;

      const key =
        date.month() >= 11 && date.date() >= 17 && date.day() > 0
          ? `advent_${date.locale('en').format('MMMM_D').toLowerCase()}`
          : `advent_${Math.floor(idx / 7) + 1}_${date.locale('en').format('dddd').toLowerCase()}`;
      ...
      const calendar = this.#computeCalendarMetadata(
        year,
        date,
        datesOfAdvent[0],
      ...
      acc[key] = new LiturgicalDay(
        {
          ...
          periods: [],
          cycles: this.#addLiturgicalCycleMetadata(date, calendar),
          calendar,
  ...
  christmasTimeBuilder = (
    ...
    const baptismOfTheLord = Dates.baptismOfTheLord(year, this.#config.epiphanyOnSunday);
    const datesIndex: DatesIndex = {};
    ...
    datesOfChristmasTime.forEach((date: Dayjs, idx) => {
      ...
      // Days within the Octave of Christmas. (UNLY #59 9)
        ...
        name = this.#config.i18next.t('roman_rite:seasons.christmas_time.octave', {
          count: idx + 1,
        });
      ...
      // Second Sunday after Christmas.
        ...
        name = this.#config.i18next.t(`roman_rite:seasons.christmas_time.${key}`);
      ...
      // Days before Epiphany
        ...
        key = `christmas_time_${date.locale('en').format('MMMM_D').toLowerCase()}`;
        name = this.#config.i18next.t('roman_rite:seasons.christmas_time.before_epiphany', {
          day: date.locale(this.#config.localeKey).format('dddd'),
        });
      ...
      // Days after Epiphany
        ...
        name = this.#config.i18next.t('roman_rite:seasons.christmas_time.after_epiphany', {
          day: date.locale(this.#config.localeKey).format('dddd'),
        });
        precedence = Precedences.Weekday_13;
        periods.push(LiturgicalPeriods.DAYS_AFTER_EPIPHANY);
      ...
      if (
        ...
        const calendar = this.#computeCalendarMetadata(
          ...
          this.#getWeekOfChristmasTime(date),
          idx,
        );
  ...
  lentBuilder = (year: number): LiturgicalDefBuiltData => {
    ...
    const byKeys = datesOfLent.reduce((acc: Record<string, LiturgicalDay>, date: Dayjs, idx) => {
      const week = Math.floor((idx - 4) / 7) + 1;
      let key: string;
      let name: string;
      let precedence: Precedences;
      const seasons = [LiturgicalSeasons.LENT];
      const periods = [LiturgicalPeriods.PRESENTATION_OF_THE_LORD_TO_HOLY_THURSDAY];
      ...
      // Days after Ash Wednesday
        ...
        key = `${date.locale('en').format('dddd').toLowerCase()}_after_ash_wednesday`;
        precedence = Precedences.PrivilegedWeekday_9;
        name = this.#config.i18next.t('roman_rite:seasons.lent.day_after_ash_wed', {
          day: date.locale(this.#config.localeKey).format('dddd'),
        });
      }
      ...
      // Holy Week. (UNLY #30)
        ...
        precedence = Precedences.PrivilegedWeekday_9;
        name = this.#config.i18next.t('roman_rite:seasons.lent.holy_week_day', {
          day: date.locale(this.#config.localeKey).format('dddd'),
        });
      ...
      // Weekdays of Lent. (UNLY #59 9)
        ...
        key = `lent_${week}_${date.locale('en').format('dddd').toLowerCase()}`;
        precedence = Precedences.PrivilegedWeekday_9;
  ...
  paschalTriduumBuilder = (year: number): LiturgicalDefBuiltData => {
    ...
    const datesOfPaschalTriduum = Dates.datesOfPaschalTriduum(year, false).slice(0, -1);
    const datesIndex: DatesIndex = {};
        ...
        acc[key] = new LiturgicalDay(
          {
            ...
            cycles: this.#addLiturgicalCycleMetadata(date, calendar),
            calendar,
            liturgicalColors: idx === 0 ? [LiturgicalColors.RED] : [LiturgicalColors.WHITE],
  ...
  easterTimeBuilder = (year: number): LiturgicalDefBuiltData => {
    ...
    const byKeys = datesOfEaster.reduce((acc: Record<string, LiturgicalDay>, date: Dayjs, idx) => {
      const week = Math.floor(idx / 7) + 1;
      let key: string;
      let name: string;
      let precedence: Precedences;
      ...
      // Sundays of Easter Time. (UNLY #59 2)
        ...
        precedence = Precedences.PrivilegedSunday_2;
        name = this.#config.i18next.t('roman_rite:seasons.easter_time.sunday', {
          week: idx + 1,
        });
      }
      ...
      // Days within the Octave of Easter. (UNLY #59 2)
        ...
        periods.push(LiturgicalPeriods.EASTER_OCTAVE);
        name = this.#config.i18next.t('roman_rite:seasons.easter_time.octave', {
          day: date.locale(this.#config.localeKey).format('dddd'),
        });
      }
      ...
      // Saturday before Pentecost. (UNLY #59 13)
        ...
        key = `easter_time_${week}_${date.locale('en').format('dddd').toLowerCase()}`;
        precedence = Precedences.Weekday_13;
        name = this.#config.i18next.t('roman_rite:seasons.easter_time.weekday', {
          day: date.locale(this.#config.localeKey).format('dddd'),
          week: idx + 1,
        });
      }
  ...
  earlyOrdinaryTimeBuilder = (year: number): LiturgicalDefBuiltData =>
    this.#ordinaryTimeBuilderFromDates(
      year,
      Dates.datesOfEarlyOrdinaryTime(year, this.#config.epiphanyOnSunday),
  ...
  /**
   * Build dates of the Ordinary Time
   * @private
   * @param year
   * @param dates Dates from the early/late Ordinary Time
   * @param periodOfOrdinaryTime
   * @param isEarlyOrdinaryTime
   */
  #ordinaryTimeBuilderFromDates = (
    year: number,
    dates: Dayjs[],
    ...
    const lastDayOfSeason = Dates.firstSundayOfAdvent(this.#config.year + 1).subtract(1, 'day');

    const byKeys = dates.reduce((acc: Record<string, LiturgicalDay>, date: Dayjs, idx) => {
      ...
      const name =
        date.day() === 0
          ...
          : this.#config.i18next.t('roman_rite:seasons.ordinary_time.weekday', {
              day: date.locale(this.#config.localeKey).format('dddd'),
              week: calendar.weekOfSeason,
            });
