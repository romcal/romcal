...
describe('createLogger', () => {
  // Grouping reads GITHUB_ACTIONS, so the tests below have to state which side of it
  // they are on. Left to the ambient environment they pass locally and fail on CI,
  // where the logger emits workflow commands instead of indenting.
  const ambient = process.env.GITHUB_ACTIONS;
  beforeEach(() => {
    delete process.env.GITHUB_ACTIONS;
  });
  afterAll(() => {
    if (ambient !== undefined) process.env.GITHUB_ACTIONS = ambient;
  });
