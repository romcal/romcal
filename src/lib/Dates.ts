...
// Seidelmann, editor.
// year: The year on which to check when Easter Sunday falls (integer)
const _easterSunday = (year: number): Record<string, number> => {
  const Y = year;
...
const easterSunday = (year: number): dayjs.Dayjs => {
  const { month, day } = _easterSunday(year);
  return dayjs.utc(`${year}-${month}-${day}`);
};

/**
 * The Solemnity of Pentecost occurs 49 days after Easter Sunday.
 *
 * @param year The year to use for the calculation
 */
const pentecostSunday = (year: number): dayjs.Dayjs => easterSunday(year).add(49, 'day').startOf('day');

/**
 * Divine Mercy Sunday is celebrated on the Sunday after Easter Sunday, the Octave of Easter,
 * observed by Roman Catholic as well as some Anglicans
 *
 * @param year The year to use for the calculation
 */
const divineMercySunday = (year: number): dayjs.Dayjs => easterSunday(year).add(7, 'day').startOf('day');
 ...
 * which are not days of fast, are excluded) before Easter Sunday and
 * can fall as early as 4 February or as late as 10 March.
 *
 * @param year The year to use for the calculation
 */
const ashWednesday = (year: number): dayjs.Dayjs => easterSunday(year).subtract(46, 'day').startOf('day');

// Maundy Thursday (also known as Holy Thursday) is
// the Christian holy day falling on the Thursday before Easter.
// y: year
const holyThursday = (year: number): dayjs.Dayjs => easterSunday(year).subtract(3, 'day').startOf('day');

// Palm Sunday is a Christian movable feast that
// falls on the Sunday before Easter Sunday.
// y: year
const palmSunday = (year: number): dayjs.Dayjs => easterSunday(year).subtract(7, 'day').startOf('day');
 ...
 * @param year The year to use for the calculation
 */
const holySaturday = (year: number): dayjs.Dayjs => easterSunday(year).subtract(1, 'day').startOf('day');
...
/**
 * Holy Week is the week just before Easter Sunday.
...
// y: year
const goodFriday = (year: number): dayjs.Dayjs => easterSunday(year).subtract(2, 'day').startOf('day');
...
// y: year
const josephSpouseOfMary = (year: number): dayjs.Dayjs => {
...
// y: year
const nativityOfJohnTheBaptist = (year: number): dayjs.Dayjs => dayjs.utc(`${year}-6-24`);
...
const ascension = (year: number, ascensionOn7thSundayOfEaster = false): dayjs.Dayjs => {
  ...
  if (!isNil(ascensionOn7thSundayOfEaster) && ascensionOn7thSundayOfEaster) {
    return easterSunday(year).add(42, 'day');
  }
  // else by default, Ascension on Thursday
  else {
    return easterSunday(year).add(39, 'day');
  }
};

// The Solemnity of Trinity Sunday occurs 56 days after Easter.
// y: Takes the year (integer)
const trinitySunday = (year: number): dayjs.Dayjs => easterSunday(year).add(56, 'day');
...
const corpusChristi = (year: number, corpusChristiOnSunday = true): dayjs.Dayjs => {
  ...
  if (!isNil(corpusChristiOnSunday) && corpusChristiOnSunday) {
    return easterSunday(year).add(63, 'day');
  } else {
    // If specified, move Corpus Christi to Thursday
    return easterSunday(year).add(60, 'day');
  }
};

// The Solemnity of the Sacred Heart of Jesus occurs 68 days after Easter.
// y: year
const sacredHeartOfJesus = (year: number): dayjs.Dayjs => easterSunday(year).add(68, 'day');

// Immaculate Heart of Mary occurs 69 days after
// Easter and is a Memorial. This was formerly an Optional Memorial.
// y: year
const immaculateHeartOfMary = (year: number): dayjs.Dayjs => easterSunday(year).add(69, 'day');
 ...
 * @param year The year for the calculation
 */
const exaltationOfTheHolyCross = (year: number): dayjs.Dayjs => dayjs.utc(`${year}-9-14`);
